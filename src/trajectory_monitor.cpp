...
BaseTrajectoryMonitor::BaseTrajectoryMonitor(MonitorParams params) : params_(params) {}

void BaseTrajectoryMonitor::setTrajectory(PlannedBaseTrajectory traj) {
  traj_ = std::move(traj);
  last_best_ = 0;
  diverged_accum_ = 0.0;
  have_last_update_time_ = false;
...
void BaseTrajectoryMonitor::updateTrajectory(PlannedBaseTrajectory traj) {
  const std::size_t old_last_best = last_best_;
  traj_ = std::move(traj);
  if (traj_.empty()) {
    last_best_ = 0;
...
double BaseTrajectoryMonitor::orientationErrorDeg(double yaw_des, double yaw) {
  return std::abs(normalizeAngle(yaw - yaw_des)) * 180.0 / M_PI;
}

TrackingMetrics BaseTrajectoryMonitor::update(double t_now,
  ...
  int best_k = k0;
  for (int k = k_min; k <= k_max; ++k) {
    const Eigen::Vector2d& pd = traj_.position[static_cast<std::size_t>(k)];
    const double d2 = (p_now - pd).squaredNorm();
    if (d2 < best_d2) {
      best_d2 = d2;
      best_k = k;
  ...
  const Eigen::Vector2d& p_des = traj_.position[last_best_];
  const double yaw_des = traj_.yaw[last_best_];

  m.closest_index = last_best_;
  m.progress = (N <= 1) ? 1.0 : static_cast<double>(last_best_) / static_cast<double>(N - 1);
  m.pos_err = (p_now - p_des).norm();
  m.ori_err_deg = orientationErrorDeg(yaw_des, yaw_now);
  m.lag_sec = t_now - traj_.time[last_best_];
